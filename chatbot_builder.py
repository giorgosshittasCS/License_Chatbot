...
def read_licenses2():
    licenses = []
    filenames = list_files_in_directory("newJSON/")
    for filename in filenames:
        with open("newJSON/" + filename, "r") as file:
            data = json.load(file)
...
def create_nodes(questions, positive_subsets, negative_subsets):
    ...
    while line_index < len(lines):
        ...
        if parts3[1] == "end" or parts3[1] == "none" or parts3[1] == "None":
            neutral_node = None
        else:
...
file_path = "Chatbot Questions/questions_json.json"
